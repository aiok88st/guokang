...
$exc = new exchange($ecs->table("doctor_cat"), $db, 'cat_id', 'cat_name');
/* act操作项的初始化 */
...
{
    $articlecat = doctor_cat_list(0, 0, false);
    foreach ($articlecat as $key => $cat)
    {
        $articlecat[$key]['type_name'] = '普通分类';
    }
    $smarty->assign('ur_here',     '科室分类');
    $smarty->assign('action_link', array('text' => '添加科室分类', 'href' => 'doctorcat.php?act=add'));
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('doctorcat_list.htm'));
}
...
{
    /* 权限判断 */
    admin_priv('doctor_cat');
    $smarty->assign('cat_select',  doctor_cat_list(0));
    $smarty->assign('ur_here',     '添加科室分类');
    $smarty->assign('action_link', array('text' => '科室分类', 'href' => 'doctorcat.php?act=list'));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $sql = "SELECT cat_type FROM " . $ecs->table('doctor_cat') . " WHERE cat_id = '$_POST[parent_id]'";
    ...
    $sql = "INSERT INTO ".$ecs->table('doctor_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav, cat_link)
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]', '$_POST[cat_link]')";
    $db->query($sql);
    ...
    if($_POST['show_in_nav'] == 1)
    {
        ...
        $vieworder += 2;
        //显示在自定义导航栏中
        $sql = "INSERT INTO " . $ecs->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $_POST['cat_name'] . "', 'a', '" . $db->insert_id() . "','1','$vieworder','0', '" . build_uri('doctor_cat', array('acid'=> $db->insert_id()), $_POST['cat_name']) . "','middle')";
        $db->query($sql);
    }

    admin_log($_POST['cat_name'],'add','doctorcat');

    $link[0]['text'] = $_LANG['continue_add'];
    $link[0]['href'] = 'doctorcat.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'doctorcat.php?act=list';
    clear_cache_files();
    sys_msg($_POST['cat_name'].'分类添加成功',0, $link);
}
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name, cat_type, cat_desc, show_in_nav, keywords, parent_id,sort_order,cat_link FROM ".
        $ecs->table('doctor_cat'). " WHERE cat_id='$_REQUEST[id]'";
    $cat = $db->GetRow($sql);
    ...
    $options    =   doctor_cat_list(0, $cat['parent_id'], false);
    $select     =   '';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $row = $db->getRow("SELECT cat_type, parent_id FROM " . $ecs->table('doctor_cat') . " WHERE cat_id='$_POST[id]'");
    ...
    /* 检查设定的分类的父分类是否合法 */
    $child_cat = doctor_cat_list($_POST['id'], 0, false);
    if (!empty($child_cat))
    {
    ...
    $dat = $db->getOne("SELECT cat_name, show_in_nav FROM ". $ecs->table('doctor_cat') . " WHERE cat_id = '" . $_POST['id'] . "'");
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]', cat_link = '$_POST[cat_link]'",  $_POST['id']))
    {
        ...
        if($_POST['show_in_nav'] != $dat['show_in_nav'])
        {
            if($_POST['show_in_nav'] == 1)
            {
                ...
                if(empty($nid))
                {
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $ecs->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                    $uri = build_uri('doctor_cat', array('acid'=> $_POST['id']), $_POST['cat_name']);
                    //不存在
        ...
        $link[0]['href'] = 'doctorcat.php?act=list';
        $note = '修改成功';
        admin_log($_POST['cat_name'], 'edit', 'doctorcat');
        clear_cache_files();
        sys_msg($note, 0, $link);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT cat_type FROM " . $ecs->table('doctor_cat') . " WHERE cat_id = '$id'";
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('doctor_cat') . " WHERE parent_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
        /* 还有子分类，不能删除 */
    ...
    /* 非空的分类不允许删除 */
    $sql = "SELECT COUNT(*) FROM ".$ecs->table('doctor')." WHERE cat_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
    ...
    $url = 'doctorcat.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
